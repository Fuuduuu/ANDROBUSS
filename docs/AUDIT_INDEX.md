...
| PASS 33 / `3b4a82e` | Origin search dialog over runtime-backed candidates | `docs/audit/PASS_33_ORIGIN_SEARCH_DIALOG.md` | accepted | Need searchable origin-group selection with explicit multi-option handling and no GPS/network scope expansion |
| PASS UI-02 / `e2c21b6` | Search screen flow polish (UI/copy/state only) | `docs/audit/PASS_UI_02_SEARCH_SCREEN_FLOW_POLISH.md` | accepted | Need stale destination gating, origin clutter reduction, and rider-facing wording polish without ViewModel/domain/data/runtime changes |
| PASS FEED-01 / `7913eb5` | Feed freshness and update architecture docs-only decision | `docs/audit/PASS_FEED_01_FEED_FRESHNESS_AND_UPDATE_ARCHITECTURE_DOCS.md` | accepted | Need freshness policy, metadata model, activation/swap policy, and FEED-02/03/04 sequencing before implementation |
| PASS FEED-02 / `589ade4` | Room schema export and feed metadata entity foundation | `docs/audit/PASS_FEED_02_ROOM_SCHEMA_EXPORT_AND_FEED_METADATA_ENTITY.md` | accepted | Need Room v2 metadata table + migration + schema export before downloader pipeline work |
| PASS AUTO-08 / current | Feed drift and boundary check after FEED-02 | `docs/audit/PASS_AUTO_08_FEED_DRIFT_AND_BOUNDARY_CHECK.md` | current candidate | Need docs/state sync + boundary verification before FEED-03 downloader/import implementation |
