...
| PASS 28A / uncommitted | App SearchViewModel feed + destination state baseline | `docs/audit/PASS_28A_APP_SEARCH_VIEWMODEL_FEED_AND_DESTINATION_STATE.md` | source accepted | Need first app-layer feed-ready and destination resolution state before route-query/UI wiring |
| PASS 28B / current | Route query with explicit origin state baseline | `docs/audit/PASS_28B_ROUTE_QUERY_WITH_EXPLICIT_ORIGIN.md` | current candidate | Need route-query precondition states and explicit-origin behavior before Compose UI scope |
